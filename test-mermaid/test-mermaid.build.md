...
and deploys argocd and root app onto it"]
    ...
    sends the venafi credentials to the managed clusters"]
    github-auth-policy-configuration["github-auth-policy-configuration ACM policy 
    sends the github credentials to the managed clusters"]
    secured-cluster-policy["secured-cluster-policy ACM policy 
    sends the ACS bundle to the managed clusters"]
  end
  ...
  subgraph "Managed cluster"
    ...
    to create venafi token from venafi credentials, 
    ...
    to vault for extracting infra secrets"]
    nmstate-operator --> nmstate-configuration["nmstate-configuration allows access 
    to storage network"]
    namespace-config-operator --> namespace-configuration["namespace-configuration deploys 
    ESO secret store to infra namespaces"]
